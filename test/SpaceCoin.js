...
describe("SpaceCoin contract", function () {
  ...
  beforeEach(async function () {
    ...
    [owner, treas, addr1, addr2, ...addrs] = await ethers.getSigners();
  ...
  describe("Minting", function () {
    ...
    it("Can only be called by owner", async function () {
        await expect(spaceCoin.connect(addr1).increaseSupply(initialSupply)).to.be.revertedWith("NOT_OWNER");
    })

    it("New minted amount added on to supply", async function () {
        await spaceCoin.increaseSupply('300000000000000000000000');
        expect(await spaceCoin.totalSupply()).to.equal('500000000000000000000000');
        expect(await spaceCoin.balanceOf(owner.address)).to.equal('500000000000000000000000');
    })

    it("Cannot exceed maximum amount of tokens", async function () {
        await expect(spaceCoin.increaseSupply('400000000000000000000000')).to.be.revertedWith("OVER_MAX");
    })
  })
  ...
  describe("Setting tax", function () {
    ...
    it("Can only be called by owner", async function () {
        await expect(spaceCoin.connect(addr1).setTax(true)).to.be.revertedWith("NOT_OWNER");
    })
  ...
  describe("Transferring", function () {
    ...
    it("2 percent tax", async function () {
        ...
        expect(await spaceCoin.balanceOf(owner.address)).to.equal('100000000000000000000000');
        expect(await spaceCoin.balanceOf(addr1.address)).to.equal('98000000000000000000000');
        expect(await spaceCoin.balanceOf(spaceCoin.treasury())).to.equal('2000000000000000000000');
    })

    it("Without tax", async function () {
      await spaceCoin.transfer(addr1.address, '100000000000000000000000');
      expect(await spaceCoin.balanceOf(owner.address)).to.equal('100000000000000000000000');
      expect(await spaceCoin.balanceOf(addr1.address)).to.equal('100000000000000000000000');
