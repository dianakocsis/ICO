...
import {SpaceCoin__factory, SpaceCoin, ICO__factory, ICO} from "../typechain-types";
...
describe("SpaceCoin", function() {
    ...
    let addr1: SignerWithAddress, addr2: SignerWithAddress, addr3: SignerWithAddress, addr4: SignerWithAddress, addr5: SignerWithAddress;

    const tokens = (count: string) => ethers.parseUnits(count, 18)

    this.beforeEach(async function() {
        [owner, treasury, addr1, addr2, addr3, addr4, addr5] = await ethers.getSigners();
        SpaceCoin = (await ethers.getContractFactory("SpaceCoin")) as SpaceCoin__factory;
        spaceCoin = (await SpaceCoin.deploy(owner.address, treasury.address, [addr1, addr2, addr3, addr4, addr5]));
    ...
    it("Constructor", async function() {
        ...
        expect(await spaceCoin.balanceOf(treasury.address)).to.equal(tokens("350000"))
        expect(await spaceCoin.balanceOf(await spaceCoin.ico())).to.equal(tokens("150000"))
    })

    describe("Toggle Tax", function() {
        it("Only owner can toggle the tax", async function() {
            await expect(spaceCoin.connect(treasury).toggleTax(true)).to.be.revertedWithCustomError(spaceCoin, "OnlyOwner").withArgs(treasury.address, owner.address);
        })
        ...
        it("Cannot toggle tax to false if already false", async function() {
            await expect(spaceCoin.toggleTax(false)).to.be.revertedWithCustomError(spaceCoin, "NoChangeInTax");
        })

        it("Cannot toggle tax to true if already true", async function() {
            await spaceCoin.connect(owner).toggleTax(true);
            await expect(spaceCoin.toggleTax(true)).to.be.revertedWithCustomError(spaceCoin, "NoChangeInTax");
        })
    })
    ...
    describe("Transfer", function() {
        it("Normal transfer when tax is not enabled", async function() {
            ...
            expect(await spaceCoin.balanceOf(treasury.address)).to.equal(tokens("300000"))
            expect(await spaceCoin.balanceOf(addr1)).to.equal(tokens("50000"))
        })
        it("Transfer when tax is enabled", async function() {
            await spaceCoin.connect(treasury).transfer(owner.address, tokens("350000"));
            await spaceCoin.connect(owner).toggleTax(true);
            await spaceCoin.connect(owner).transfer(addr1, tokens("350000"));
            expect(await spaceCoin.balanceOf(treasury.address)).to.equal(tokens("7000"))
